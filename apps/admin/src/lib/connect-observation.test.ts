...
describe("Connect metadata projection", () => {
  it("keeps probe and process evidence separate from activity", () => {
    const value = projectConnectStatus(
      {
        auth_runtime: {
          connect_api: "running",
          tailnet_heartbeat: "200",
          launchd: "manual_probe",
        },
      },
      at,
      now,
  ...
  it("drops unknown fields and arbitrary field values", () => {
    const value = projectConnectStatus(
      {
        secret: "private",
        auth_runtime: { detail: "private", connect_sync: "private" },
      },
      at,
      now,
    );
  ...
  it("preserves unavailable probes without inventing a disconnect", () => {
    const value = projectConnectStatus(
      { auth_runtime: { tailnet_heartbeat: "000" } },
      at,
      now,
    );
  ...
  it("rejects missing sources and future observation times", () => {
    ...
    expect(() =>
      projectConnectStatus({ auth_runtime: {} }, at, now - 1),
    ).toThrow();
  });
});
