...
- `admin-access-policy.ts` keeps an explicit public auth/static allowlist. Development bypass is read-only GET/HEAD, requires development mode, and requires a plain HTTP loopback origin and an explicit page/asset pattern. Six Life section previews and Observability are development page exceptions only; their production routes and protected APIs remain protected. The helper is one layer, not evidence that public endpoints themselves implement all required checks.
