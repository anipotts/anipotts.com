...
- The server loader requests editorial storage only, preserving successful private snapshot reads when other reads reject. It does not query Operations or Life. `writing-inventory.ts` was a compatibility wrapper over the canonical projection. It was removed as a confirmed orphan on 2026-09-14 (issue #358 knip audit): knip and git grep found no importer.
...
SHA-256 at audit time. A subsequent edit requires renewed review of that file; this document does not mark unresolved findings complete. `writing-inventory.ts` was removed as a confirmed orphan on 2026-09-14; its row records the audited bytes of the deleted file.
